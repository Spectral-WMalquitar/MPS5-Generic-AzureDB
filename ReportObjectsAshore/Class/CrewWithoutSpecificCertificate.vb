...
Public Class CrewWithoutSpecificCertificate

    Private MainReport As New rptCrewWithoutASpecificCertificate

    Public Sub New(ByRef REPORT_DETAIL As ReportDetail)
        Dim cSQL As String
        'Dim cSQL_AllCrewAllCertificate As String
        ...
        'cFilter = IIf(WhereList.Length > 0, "FKeyCertificate IN " & WhereList, "")

        'Select Case REPORT_DETAIL.FilterOption.GetFilterValue("Certificate Status")
        '    Case "Planned"
        '        cFilter = cFilter & IIf(cFilter.Length > 0, " AND ", "") & " CertificateStatus = 1 "
        '    Case "Required"
        '        cFilter = cFilter & IIf(cFilter.Length > 0, " AND ", "") & " CertificateStatus = 2 "
        '    Case "Completed"
        '        cFilter = cFilter & IIf(cFilter.Length > 0, " AND ", "") & " CertificateStatus = 3 "
        'End Select

        'cSQL_AllCrewAllCertificate = "" & _
        '        "SELECT         admcor.name AS Certificatename, " & _
        '        "               admcor.pkey as Certificatecode, " & _
        '        "               crew.* " & _
        '        "FROM		    dbo.Rpt_BioInfo AS crew, " & _
        '        "               dbo.tbladmCertificate as admcor " & _
        '        "WHERE          admcor.pkey IN " & WhereList

        'cSQL = "SELECT          q.* " & _
        '       "FROM            (" & cSQL_AllCrewAllCertificate & ") q LEFT OUTER JOIN " & _
        '       "                dbo.Rpt_CrewTraining as cc ON q.idnbr = cc.idnbr AND q.Certificatecode = cc.fkeyCertificate " & _
        '       "WHERE           cc.pkey is null AND ActivityType = 'ASHORE' " & _
        '                        IIf(MainReportFilter.Length > 0, " AND " & MainReportFilter, "")

        cSQL = "Select * from Rpt_CrewWithoutDocRpt where FKeyDocGroup = 'SYSLICCERT' and " & IIf(MainReportFilter.Length > 0, MainReportFilter, "")
