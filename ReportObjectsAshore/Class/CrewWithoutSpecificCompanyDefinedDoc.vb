...
Public Class CrewWithoutSpecificCompanyDefinedDoc

    Private MainReport As New rptCrewWithoutSpecificCompanyDefinedDoc

    Public Sub New(ByRef REPORT_DETAIL As ReportDetail)
        Dim cSQL As String
        'Dim cSQL_AllCrewAllTravel As String
        ...
        'cFilter = IIf(WhereList.Length > 0, "FKeyTravel IN " & WhereList, "")

        'Select Case REPORT_DETAIL.FilterOption.GetFilterValue("Travel Status")
        '    Case "Planned"
        '        cFilter = cFilter & IIf(cFilter.Length > 0, " AND ", "") & " TravelStatus = 1 "
        '    Case "Required"
        '        cFilter = cFilter & IIf(cFilter.Length > 0, " AND ", "") & " TravelStatus = 2 "
        '    Case "Completed"
        '        cFilter = cFilter & IIf(cFilter.Length > 0, " AND ", "") & " TravelStatus = 3 "
        'End Select

        'cSQL_AllCrewAllTravel = "" & _
        '        "SELECT         admcor.name AS Travelname, " & _
        '        "               admcor.pkey as Travelcode, " & _
        '        "               crew.* " & _
        '        "FROM		    dbo.Rpt_BioInfo AS crew, " & _
        '        "               dbo.tbladmTravel as admcor " & _
        '        "WHERE          admcor.pkey IN " & WhereList

        'cSQL = "SELECT          q.* " & _
        '       "FROM            (" & cSQL_AllCrewAllTravel & ") q LEFT OUTER JOIN " & _
        '       "                dbo.Rpt_CrewTraining as cc ON q.idnbr = cc.idnbr AND q.Travelcode = cc.fkeyTravel " & _
        '       "WHERE           cc.pkey is null AND ActivityType = 'ASHORE' " & _
        '                        IIf(MainReportFilter.Length > 0, " AND " & MainReportFilter, "")

        cSQL = "Select * from Rpt_CrewWithoutDocRpt where FKeyDocGroup = 'SYSCOMPDEF' and " & IIf(MainReportFilter.Length > 0, MainReportFilter, "")
