...
#Region "Easy Edit"
    Private FormName As String = "Admin Rank"
...
#End Region
    ...
    Public Overrides Sub RefreshData()
        TableName = "tblAdmRank"
        'temporary
        'SetDeleteSubVisibility(Name, DevExpress.XtraBars.BarItemVisibility.Never)

        strRequiredFields = "txtName;txtAbbrv;cboDeptCode;cboFKeyRankGrp;cboFKeyRankType"
        MyBase.RefreshData()
        Me.header.Text = IIf(strDesc = "New Record", strDesc, "Rank Details - " & strDesc)
        If Not bLoaded Then
            initControls()
            bLoaded = True
        End If
        ...
        Else
            ...
            Me.txtChineseName.Text = Trim(IfNull(blList.GetFocusedRowData("ChineseName"), ""))
            Me.cboDeptCode.EditValue = blList.GetFocusedRowData("DeptCode")
            Me.cboFKeyRankGrp.EditValue = blList.GetFocusedRowData("FKeyRankGrp")
            Me.cboFKeyRankType.EditValue = blList.GetFocusedRowData("FKeyRankType")
            Me.txtRemarks.Text = Trim(IfNull(blList.GetFocusedRowData("Remarks"), ""))
            'Me.txtRegistryCode.Text = Trim(IfNull(blList.GetFocusedRowData("RegistryCode"), ""))
    ...
    Public Overrides Sub SaveData()
        ...
        If ValidateFields(New DevExpress.XtraEditors.TextEdit() {txtName, txtAbbrv, cboDeptCode, cboFKeyRankType, cboFKeyRankGrp}) Then
            ...
            LastUpdatedBy = clsAudit.AssembleLastUBy(USER_NAME, "Rank", 10, System.Environment.MachineName, Me.txtName.EditValue, FormName)
            If bAddMode Then
            ...
            Else
                ...
                Else
                    ...
                    info = UpdateRelatedAdminName(strID, TableName, GenerateUpdateScript(Me.LayoutControl1.Root, 3, Me.TableName, "LastUpdatedBy='" & LastUpdatedBy & "'", "PKey='" & strID & "'"))
                    BRECORDUPDATEDs = False
                    blList.RefreshData()
                    type = "Updated"
                End If
    ...
    Public Overrides Function CheckValidateFields(Optional showMsg As Boolean = True) As Boolean
        ...
        If res = MsgBoxResult.Yes Then
            If ValidateFields(New DevExpress.XtraEditors.TextEdit() {txtName, txtAbbrv, cboDeptCode, cboFKeyRankType, cboFKeyRankGrp}, showMsg) Then
                If bAddMode Then
                    If CheckDuplicate(Me.TableName, New DevExpress.XtraEditors.TextEdit() {txtName, txtAbbrv}) Then
                        flag = False
                        ALLOWNEXTS = flag
    ...
    Private Sub initControls()
        ...
        Me.cboDeptCode.Properties.DataSource = DB.CreateTable("SELECT * FROM tblAdmDept ORDER BY Name ASC, SortCode ASC")
        Me.cboFKeyRankGrp.Properties.DataSource = DB.CreateTable("SELECT * FROM tblAdmRankGrp ORDER BY Name ASC, SortCode ASC")
        Me.cboFKeyRankType.Properties.DataSource = DB.CreateTable("SELECT * FROM tbladmRankType ORDER BY Name ASC, SortCode ASC")
    End Sub

    Private Sub cboDeptCode_ButtonClick(sender As System.Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles cboDeptCode.ButtonClick
        ClearLookUpEdit(sender, e)
    End Sub

    Private Sub cboFKeyRankGrp_ButtonClick(sender As System.Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles cboFKeyRankGrp.ButtonClick
        ClearLookUpEdit(sender, e)
    End Sub

    Private Sub cboFKeyRankType_ButtonClick(sender As System.Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles cboFKeyRankType.ButtonClick
        ClearLookUpEdit(sender, e)
    End Sub
End Class
